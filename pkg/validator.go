...
func (qv *QuotaValidator) Handle(quota string,history OrderHistory, newOrder string ) error {
	...
	switch quota {
		case Basic: return qv.validate(history,newOrder,qv.RulesContainer.GetProductRules(Basic,newOrder))
		case Maniac: return qv.validate(history,newOrder,qv.RulesContainer.GetProductRules(Maniac,newOrder))
		case Lover: return qv.validate(history,newOrder,qv.RulesContainer.GetProductRules(Lover,newOrder))
	}
